...
@interface FMFuncItemCollectionViewCell ()
@property (nonatomic, strong) UIImageView *iconImageView;
@property (nonatomic, strong) UILabel *titleLabel;
@end

@implementation FMFuncItemCollectionViewCell
...
@interface FMCollectionViewController ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
CGFloat fm_FuncAnimationValue = 0.05;
static NSString *funcItemCollectionViewCellId = @"FMFuncItemCollectionViewCellId";
...
//    [self.cancelButton d3_setRotate:180 duration:fm_FuncAnimationValue*self.dataArray.count+0.1 completion:nil];
}
...
- (UICollectionView *)collectionView {
    ...
    [collectionView registerClass:[FMFuncItemCollectionViewCell class] forCellWithReuseIdentifier:funcItemCollectionViewCellId];
    return _collectionView;
}
...
- (UIView *)cancelView {
    ...
    _cancelView = [[UIView alloc]initWithFrame:CGRectMake(0, self.view.frame.size.height - 50, self.view.frame.size.width, 45)];
    [self.view addSubview:_cancelView];
    
    UIButton *cancelButton;
    cancelButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0.5, self.view.frame.size.width, 49)];
    [cancelButton setTitle:@"取消" forState:UIControlStateNormal];
    [cancelButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [cancelButton setBackgroundColor:[UIColor colorWithRed:0.97 green:0.30 blue:0.30 alpha:1.00]];
...
- (void)setupData {
    if ([FMFuncItemManager shareInstance].dataArray) {
        self.dataArray = [FMFuncItemManager shareInstance].dataArray;
    }
}
...
//    [sender d3_setRotate:180 duration:fm_FuncAnimationValue*self.dataArray.count+0.1 completion:^{
//    }];
...
- (void)closeCollectionViewController {
    ...
    [FMFuncItemManager closeCollectionViewController];
}

- (void)closeCollectionViewControllerWithCompletion:(void(^)(id objc))completion {
    //[FMSuspensionManager destroyWindowForKey:kYBFuncItemCollectionViewControllerKey];
    [FMFuncItemManager closeCollectionViewControllerWithCompletion:^(id objc) {
        if (completion) {
            completion(objc);
        }
    }];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    FMFuncItemCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:funcItemCollectionViewCellId forIndexPath:indexPath];
    
    FMItemDataModel *dataModel = self.dataArray[indexPath.row];
    ...
    if (!self.isCancelStatus) {
        ...
        [UIView animateWithDuration: 0.5 delay: fm_FuncAnimationValue * indexPath.item usingSpringWithDamping: 0.65 initialSpringVelocity:1 options: UIViewAnimationOptionCurveLinear animations: ^{
            cell.center = center;
        } completion: nil];
    }else {
        CGPoint center = cell.center;
        
        [UIView animateWithDuration: 0.5 delay: fm_FuncAnimationValue*(self.dataArray.count - indexPath.item) usingSpringWithDamping: 0.65 initialSpringVelocity:1 options: UIViewAnimationOptionCurveLinear animations: ^{
            CGPoint startCenter = center;
            startCenter.y += CGRectGetMaxY(collectionView.frame);
            cell.center = startCenter;
        } completion: nil];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    //FMFuncItemCollectionViewCell *cell = (FMFuncItemCollectionViewCell *)[collectionView cellForItemAtIndexPath:indexPath];
    FMItemDataModel *dataModel = self.dataArray[indexPath.row];
    ...
    [self closeCollectionViewControllerWithCompletion:^(id objc) {

    }];
}

- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    //FMFuncItemCollectionViewCell *newCell = (FMFuncItemCollectionViewCell *)cell;
