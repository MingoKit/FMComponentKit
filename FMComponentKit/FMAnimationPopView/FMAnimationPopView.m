...
@interface FMAnimationPopView ()<UIGestureRecognizerDelegate>
...
/** 显示时动画弹框样式 */
@property (nonatomic) FMAnimationPopStyle animationPopStyle;
/** 移除时动画弹框样式 */
@property (nonatomic) FMAnimationDismissStyle animationDismissStyle;
/** 显示时背景是否透明，透明度是否为<= 0，默认为NO */
@property (nonatomic) BOOL isTransparent;

@end

@implementation FMAnimationPopView
...
- (void)pop
{
    ...
    if (self.animationPopStyle == FMAnimationPopStyleNO) {
        self.alpha = 0.0;
...
- (void)dismiss
{
    ...
    if (self.animationDismissStyle == FMAnimationPopStyleNO) {
        [UIView animateWithDuration:duration animations:^{
            ws.alpha = 0.0;
...
- (void)hanlePopAnimationWithDuration:(NSTimeInterval)duration
{
    ...
    switch (self.animationPopStyle) {
        case FMAnimationPopStyleScale:
        {
            ...
            break;
        case FMAnimationPopStyleShakeFromTop:
        case FMAnimationPopStyleShakeFromBottom:
        case FMAnimationPopStyleShakeFromLeft:
        case FMAnimationPopStyleShakeFromRight:
        {
            CGPoint startPosition = self.contentView.layer.position;
            if (self.animationPopStyle == FMAnimationPopStyleShakeFromTop) {
                self.contentView.layer.position = CGPointMake(startPosition.x, -startPosition.y);
            } else if (self.animationPopStyle == FMAnimationPopStyleShakeFromBottom) {
                self.contentView.layer.position = CGPointMake(startPosition.x, CGRectGetMaxY(self.frame) + startPosition.y);
            } else if (self.animationPopStyle == FMAnimationPopStyleShakeFromLeft) {
                self.contentView.layer.position = CGPointMake(-startPosition.x, startPosition.y);
            } else {
                self.contentView.layer.position = CGPointMake(CGRectGetMaxX(self.frame) + startPosition.x, startPosition.y);
            }

            [UIView animateWithDuration:duration*0.6 delay:0 usingSpringWithDamping:1.0 initialSpringVelocity:1.0 options:UIViewAnimationOptionCurveEaseIn animations:^{
                ws.contentView.layer.position = startPosition;
            } completion:nil];
        }
            break;
        case FMAnimationPopStyleCardDropFromLeft:
        case FMAnimationPopStyleCardDropFromRight:
        {
            CGPoint startPosition = self.contentView.layer.position;
            if (self.animationPopStyle == FMAnimationPopStyleCardDropFromLeft) {
            ...
            [UIView animateWithDuration:duration*0.6 animations:^{
                ws.contentView.layer.transform = CATransform3DMakeRotation(DEGREES_TO_RADIANS((ws.animationPopStyle == FMAnimationPopStyleCardDropFromRight) ? 5.5 : -5.5), 0, 0, 0);
            } completion:^(BOOL finished) {
                [UIView animateWithDuration:duration*0.2 animations:^{
                    ws.contentView.transform = CGAffineTransformMakeRotation(DEGREES_TO_RADIANS((ws.animationPopStyle == FMAnimationPopStyleCardDropFromRight) ? -1.0 : 1.0));
                } completion:^(BOOL finished) {
                    [UIView animateWithDuration:duration*0.2 animations:^{
...
- (void)hanleDismissAnimationWithDuration:(NSTimeInterval)duration
{
    ...
    switch (self.animationDismissStyle) {
        case FMAnimationDismissStyleScale:
        {
            ...
            break;
        case FMAnimationDismissStyleDropToTop:
        case FMAnimationDismissStyleDropToBottom:
        case FMAnimationDismissStyleDropToLeft:
        case FMAnimationDismissStyleDropToRight:
        {
            CGPoint startPosition = self.contentView.layer.position;
            CGPoint endPosition = self.contentView.layer.position;
            if (self.animationDismissStyle == FMAnimationDismissStyleDropToTop) {
                endPosition = CGPointMake(startPosition.x, -startPosition.y);
            } else if (self.animationDismissStyle == FMAnimationDismissStyleDropToBottom) {
                endPosition = CGPointMake(startPosition.x, CGRectGetMaxY(self.frame) + startPosition.y);
            } else if (self.animationDismissStyle == FMAnimationDismissStyleDropToLeft) {
                endPosition = CGPointMake(-startPosition.x, startPosition.y);
            } else {
            ...
            break;
        case FMAnimationDismissStyleCardDropToLeft:
        case FMAnimationDismissStyleCardDropToRight:
        {
            CGPoint startPosition = self.contentView.layer.position;
            BOOL isLandscape = UIInterfaceOrientationIsLandscape([UIApplication sharedApplication].statusBarOrientation);
            __block CGFloat rotateEndY = 0.0f;
            [UIView animateWithDuration:duration delay:0 usingSpringWithDamping:0.5 initialSpringVelocity:1.0 options:UIViewAnimationOptionCurveEaseIn animations:^{
                if (self.animationDismissStyle == FMAnimationDismissStyleCardDropToLeft) {
            ...
            break;
        case FMAnimationDismissStyleCardDropToTop:
        {
            CGPoint startPosition = self.contentView.layer.position;
...
- (NSTimeInterval)getPopDefaultDuration:(FMAnimationPopStyle)animationPopStyle
{
    ...
    } else if (animationPopStyle == FMAnimationPopStyleScale) {
        defaultDuration = 0.3f;
    } else if (animationPopStyle == FMAnimationPopStyleShakeFromTop ||
               animationPopStyle == FMAnimationPopStyleShakeFromBottom ||
               animationPopStyle == FMAnimationPopStyleShakeFromLeft ||
               animationPopStyle == FMAnimationPopStyleShakeFromRight ||
               animationPopStyle == FMAnimationPopStyleCardDropFromLeft ||
               animationPopStyle == FMAnimationPopStyleCardDropFromRight) {
...
- (NSTimeInterval)getDismissDefaultDuration:(FMAnimationDismissStyle)animationDismissStyle
{
    ...
    if (animationDismissStyle == FMAnimationDismissStyleNO) {
        defaultDuration = 0.2f;
    } else if (animationDismissStyle == FMAnimationDismissStyleScale) {
        defaultDuration = 0.2f;
    } else if (animationDismissStyle == FMAnimationDismissStyleDropToTop ||
               animationDismissStyle == FMAnimationDismissStyleDropToBottom ||
               animationDismissStyle == FMAnimationDismissStyleDropToLeft ||
               animationDismissStyle == FMAnimationDismissStyleDropToRight ||
               animationDismissStyle == FMAnimationDismissStyleCardDropToLeft ||
               animationDismissStyle == FMAnimationDismissStyleCardDropToRight ||
               animationDismissStyle == FMAnimationDismissStyleCardDropToTop) {
        defaultDuration = 0.8f;
    }
    return defaultDuration;
}
