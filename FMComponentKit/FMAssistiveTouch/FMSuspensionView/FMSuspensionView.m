...
@implementation FMSuspensionContainer
...
@implementation FMSuspensionViewController
...
@implementation FMSuspensionView
...
+ (instancetype)defaultSuspensionViewWithDelegate:(id<FMSuspensionViewDelegate>)delegate {
    FMSuspensionView *sus = [[FMSuspensionView alloc] initWithFrame:CGRectMake(-kLeanProportion * 55, 100, 55, 55)
...
- (void)handlePanGesture:(UIPanGestureRecognizer*)p {
    ...
    if(p.state == UIGestureRecognizerStateBegan) {
        ...
        if (self.isAddedWindow) {
            [FMSuspensionManager windowForKey:self.fm_md5Key].center = CGPointMake(panPoint.x, panPoint.y);
        }else {
        ...
        CGFloat minSpace = 0;
        if (self.leanType == FMSuspensionViewLeanTypeHorizontal) {
        ...
        [UIView animateWithDuration:.25 animations:^{
            if (self.isAddedWindow) {
                [FMSuspensionManager windowForKey:self.fm_md5Key].center = newCenter;
            }else {
        ...
        if (self.isCancelMove) {
            ...
            [UIView animateWithDuration:.25 animations:^{
                if (self.isAddedWindow) {
                    [FMSuspensionManager windowForKey:self.fm_md5Key].center = self.originalCenter;
                }else {
...
- (void)show {
    if ([FMSuspensionManager windowForKey:self.fm_md5Key]) return;
    
    UIWindow *currentKeyWindow = [UIApplication sharedApplication].keyWindow;
    
    FMSuspensionContainer *backWindow = [[FMSuspensionContainer alloc] initWithFrame:self.frame];
    backWindow.rootViewController = [[FMSuspensionViewController alloc] init];
    [backWindow makeKeyAndVisible];
    [FMSuspensionManager saveWindow:backWindow forKey:self.fm_md5Key];
...
- (void)removeFromScreen {
    [FMSuspensionManager destroyWindowForKey:self.fm_md5Key];
}

- (void)hiddenFromScreen:(BOOL)hidden {
    [FMSuspensionManager windowForKey:self.fm_md5Key].hidden = hidden;
}

- (void)showFromScreen {
    [FMSuspensionManager windowForKey:self.fm_md5Key].hidden = NO;
}
...
-(void)addAniamtionLikeGameCenterBubble {
    ...
    FMSuspensionContainer *backWindow = (FMSuspensionContainer *)[FMSuspensionManager windowForKey:self.fm_md5Key];
    ...
    if (animationView) {
        ...
        CGRect circleContainer = CGRectInset(animationView.frame, animationView.bounds.size.width / 2 - 5, animationView.bounds.size.width / 2 - 5);
...
#pragma mark - getter
- (FMSuspensionContainer *)containerWindow {
    return (FMSuspensionContainer *)[FMSuspensionManager windowForKey:self.fm_md5Key];
}
